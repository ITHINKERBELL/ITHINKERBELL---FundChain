import React, { useState } from 'react';885
import { project_backend, createActor } from "../../../declarations/project_backend";
...
const Login_ii: React.FC = () => {
    ...
    const handleWhoAmIClick = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        e.preventDefault();
    ...
    const handleLoginClick = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        ...
        try {
                ...
                authClient.login({
                    identityProvider:
                        process.env.DFX_NETWORK === "ic"
                            ...
                            : `http://localhost:3000`, // canister id of our internet identity service [must replace this pa, idk where to find the url]
                    onSuccess: resolve,
                });
            });
            ...
            // const newActor = createActor(process.env.CANISTER_ID_II_INTEGRATION_BACKEND, {
            //     agent,
            // });
            // setActor(newActor);
        } catch (error) {
    ...
    return (
        <main>
            ...
            <br />
            <form>
                <button id="whoAmI" onClick={handleWhoAmIClick}>Who Am I</button>
            </form>
